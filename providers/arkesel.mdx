...
| `newSession` | True if session just started |
| `msisdn` | Phone number (same as userID) |
| `userData` | User input |
...
| Field | Description |
|-------|-------------|
| `sessionID` | Echo the session ID |
| `msisdn` | Echo the phone number |
| `message` | Text to display |
| `continueSession` | True to continue, false to end |

## Session Behavior

### New Session Detection

Arkesel indicates new sessions with `newSession: true`:
...
- Sessions timeout after **2.5 minutes**
- StarHash receives no notification on timeout
- Sessions are marked as `timeout` automatically
...
| `Shortcode not found` | Wrong shortcode | Verify shortcode |
...
3. This displays when StarHash errors occur
